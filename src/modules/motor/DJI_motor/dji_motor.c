...
void dji_motor_control() {
     ...
     for (size_t i = 0; i < idx; ++i) {
         ...
         measure = motor->measure;
             if ((motor->rx_id == 0x201) || (motor->rx_id == 0x202) || (motor->rx_id == 0x203) ||
                 (motor->rx_id == 0x204)) {
//            switch (motor->rx_id) {
//                case 0x201:
...
//                            power[j] = k1 * set1[j] * set1[j] + 0.000001997 * set1[j] * rpm[j];
//                            if (set1[j] > 0) {
//                                set1[j] = (-0.000001997 * rpm[j] +
//                                           sqrt(0.000001997 * rpm[j] * 0.000001997 * rpm[j] - 4 * power[j] * k1)) /
//                                          (2 * k1);
//                            } else if (set1[j] < 0) {
//                                set1[j] = (-0.000001997 * rpm[j] -
//                                           sqrt(0.000001997 * rpm[j] * 0.000001997 * rpm[j] - 4 * power[j] * k1)) /
...
//                        send_msg[group1[j]].data[2 * num1[j] + 1] = (uint8_t) (set1[j] & 0xFF);
//                    }
//                    break;
//                }
//            }

                 set = motor->control(measure);
                 // 分组填入发送数据
